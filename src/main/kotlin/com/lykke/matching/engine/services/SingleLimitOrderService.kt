...
class SingleLimitOrderService(private val limitOrderService: GenericLimitOrderService,
    ...
    override fun processMessage(messageWrapper: MessageWrapper) {
        ...
        if (!order.checkVolume(assetsPairsHolder))  {
            ...
            rejectOrder(reservedBalance, cancelVolume, limitAsset, order, balance, limitOrdersReport, orderBook, messageWrapper, MessageStatus.TOO_SMALL_VOLUME, now, isCancelOrders)
            return
        }
        ...
        if (balance - (reservedBalance - cancelVolume) < limitVolume) {
            ...
            rejectOrder(reservedBalance, cancelVolume, limitAsset, order, balance, limitOrdersReport, orderBook, messageWrapper, MessageStatus.RESERVED_VOLUME_HIGHER_THAN_BALANCE, now, isCancelOrders)
            return
        }
        ...
        if (orderBook.leadToNegativeSpreadForClient(order)) {
            ...
            rejectOrder(reservedBalance, cancelVolume, limitAsset, order, balance, limitOrdersReport, orderBook, messageWrapper, MessageStatus.LEAD_TO_NEGATIVE_SPREAD, now, isCancelOrders)
            return
        }
    ...
    private fun rejectOrder(reservedBalance: Double, cancelVolume: Double, limitAsset: String, order: NewLimitOrder, balance: Double, limitOrdersReport: LimitOrdersReport, orderBook: AssetOrderBook, messageWrapper: MessageWrapper, status: MessageStatus, now: Date, isCancelOrders: Boolean) {
